...
const SignUpPage = () => {
  const { user, googleLogin, emailSignUp, loading } = useAuth();
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      const user = await emailSignUp(userData.email, userData.password);
      const payload = {
        name: userData?.name,
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-center gap-5">
        ...
        <button onClick={handleFacebookLogin} className="cursor-pointer flex items-center gap-3 px-6 py-3 rounded-xl bg-white border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-300 hover:bg-linear-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-300 group">
          <div className="p-1 rounded-full bg-linear-to-br from-blue-500 to-blue-600 group-hover:scale-110 transition-transform duration-300">
